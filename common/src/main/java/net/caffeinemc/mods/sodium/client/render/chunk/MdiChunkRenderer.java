...
public class MdiChunkRenderer extends ShaderChunkRenderer {
    private static final Logger LOGGER = LogUtils.getLogger();
    private final SharedQuadIndexBuffer sharedIndexBuffer;

    public MdiChunkRenderer(RenderDevice device, ChunkVertexType vertexType) {
    ...
    public void render(ChunkRenderMatrices matrices,
        ...
        LOGGER.info("Rendering with MDI backend");
        ...
        while (iterator.hasNext()) {
            ...
            var batch = region.getMdiCachedBatch(renderPass, commandList);
            batch.clear();
            ...
            if (!useIndexedTessellation) {
                ...
                var geometryIterator = renderList.sectionsWithGeometryIterator(renderPass.isTranslucent());

                if (geometryIterator != null) {
                    while (geometryIterator.hasNext()) {
                        int sectionId = geometryIterator.nextByteAsInt();
                        var pMeshData = storage.getDataPointer(sectionId);

                        for (int i = 0; i < ModelQuadFacing.COUNT; i++) {
                            indexBufferSize += SectionRenderDataUnsafe.getVertexCount(pMeshData, i);
                        }
                    }
                }

                this.sharedIndexBuffer.ensureCapacity(commandList, (indexBufferSize / 4) * 6);
            }
    ...
    private static void fillCommandBuffer(MultiDrawIndirectBuffer batch,
        ...
        var geometryIterator = renderList.sectionsWithGeometryIterator(pass.isTranslucent());
        ...
        while (geometryIterator.hasNext()) {
            int sectionIndex = geometryIterator.nextByteAsInt();
    ...
    private static void addLocalIndexedDrawCommands(MultiDrawIndirectBuffer batch, long pMeshData, int mask) {
        ...
        for (int facing = 0; facing < ModelQuadFacing.COUNT; facing++) {
            ...
            batch.addCommand(
                    UInt32.uncheckedDowncast(elementCount),
                    UInt32.uncheckedDowncast(elementOffset),
                    UInt32.uncheckedDowncast(baseVertex)
            );
     ...
     */
    @SuppressWarnings("IntegerMultiplicationImplicitCastToLong")
    private static void addSharedIndexedDrawCommands(MultiDrawIndirectBuffer batch, long pMeshData, int mask) {
        ...
        final long facingList = SectionRenderDataUnsafe.getFacingList(pMeshData);
        ...
        for (int i = 0; i <= ModelQuadFacing.COUNT; i++) {
            ...
            if (i < ModelQuadFacing.COUNT) {
                ...
                if (vertexCount != 0) {
                    var facing = (int) ((facingList >>> (i * 8)) & 0xFF);
            ...
            if (maskBit == 0) {
                if (lastMaskBit == 1) {
                    ...
                    batch.addCommand(
                            UInt32.uncheckedDowncast((groupVertexCount >> 2) * 6),
                            UInt32.uncheckedDowncast(elementOffset),
                            UInt32.uncheckedDowncast(baseVertex)
                    );
    ...
    private static void executeDrawBatch(CommandList commandList, GlTessellation tessellation, MultiDrawIndirectBuffer batch) {
        System.out.println("Executing MDI draw batch");
